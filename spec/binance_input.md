...
| **7. QuestDB 테이블 스키마 정의** | `strategies/dags/binance_history_strategy/create_table.sql`과 같은 초기화 스크립트를 작성하여 `node_id VARCHAR`, `interval INT`, `ts TIMESTAMP`, 시가/종가 등 payload 컬럼을 포함한 테이블을 생성한다.                                                                                      |        |
