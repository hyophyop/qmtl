...
Traffic splits between strategy versions are adjusted by publishing `sentinel_weight` events to the internal **ControlBus**. Operational tooling should emit these events with the target `sentinel_id` and desired `weight` (0–1). Gateways consume the ControlBus and relay updates to SDK clients via WebSocket.

## Adjusting Weights

1. Use the Ops tooling to publish a `sentinel_weight` event on the ControlBus.
2. Include the `sentinel_id` identifier and a `weight` between `0` and `1`.
3. Gateways will apply the new ratio and broadcast the change to connected clients.

## Monitoring Metrics

* **Gateway metrics:** check `gateway_sentinel_traffic_ratio{sentinel_id="v1.2.1"}` in Prometheus to confirm the live split. The metric is exposed via the Gateway's `/metrics` endpoint and is updated immediately whenever a `sentinel_weight` ControlBus event is processed.
