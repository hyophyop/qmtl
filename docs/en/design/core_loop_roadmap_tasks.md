...
- [x] Create the Core Loop contract test skeleton first: tests/e2e/core_loop scaffold (#1788) with in-proc stack/fixtures.
...
- [x] Refresh mode/domain surfaces (#1769): strip legacy tokens from CLI `--mode`/SDK docs and restate WS-first + default-safe rules in ko/en guides.
...
- [x] Add ComputeContext contract tests (#1781) — lock WS-first/as_of retention plus stale/unavailable default-safe downgrade flags in the contract suite.
...
Remaining open issues (#1762, #1763, #1769, #1781, #1788) are covered here; the Phase 0–5 cluster is now complete.
