...
이 이론은 호가창(order book)에서 가격·체결 변화에 대한 유동성 공급자의 반응이 즉각적이지 않고 지연(lag)을 가지는 현상이
특정 조건에서 누적되면 시장 구조가 임계 전이(critical transition)를 겪으며 가격 반응이 비선형적으로 증폭된다고 설명합니다.
...
QRLI_t = \frac{\text{Mean Quote Update Interval}{t,\tau}}{\text{Baseline Interval}{\text{regime}} + \epsilon}
...
이 접근은 단순 가격·거래량 지표가 아니라 유동성 공급의 반응 속도라는 시장 미시 구조적 변수를 활용하므로,
