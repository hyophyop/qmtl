...
- ko_low_hangul: 18.6% hangul, 54.5% latin — `/Users/munseungjin/workspace/research/qmtl/docs/ko/architecture/dag-manager.md`
...
- L39: * 계좌 유형, **RequiredFreeBuyingPowerPercent**(유휴 매수력 비율), 브로커별 **CanSubmit/CanExecuteOrder** 등 주문 가능 판정 API. ([[QuantConnect](https://www.lean.io/docs/v2/lean-engine/class-reference/ …
- L40: * **Order Properties / Time-in-Force**:
- L42: * GTC/GTD/IOC/FOK 등의 만기 규칙, 브로커 기본 값 설정. ([[QuantConnect](https://www.quantconnect.com/docs/v2/writing-algorithms/trading-and-orders/order-properties?utm_source=chatgpt.com)][4], [ …
- L43: * **심볼 속성(SymbolProperties / DB)**:
- L45: * **최소 호가단위(tick size)**, **로트/최소 주문수량**, **계약승수**, 종목 통화 등. 주문 호가/수량 검증·라운딩에 사용. ([[QuantConnect](https://www.lean.io/docs/v2/lean-engine/class-reference/classQuantConnect_1_1Secu …
...
- L57: * 주문 접수 전 **증거금/레버리지 가능 여부**와 최대 수량 판정. 브로커/시간대/상품에 따라 복잡한 규칙을 캡슐화. ([[QuantConnect](https://www.quantconnect.com/docs/v2/writing-algorithms/reality-modeling/buying-power?utm_sourc …
- L63: * 주문타입별(시장가/지정가/스탑/개장·폐장가 등) **체결가·체결수량** 결정. **개장/폐장 옥션가격** 사용 가능. ([[QuantConnect](https://www.quantconnect.com/docs/v2/writing-algorithms/reality-modeling/trade-fills/key-concep …
- L72: * 브로커·시장별 **거래 수수료** 구조(고정·퍼센트·브로커 특화). 체결 이벤트에 비용 반영. ([[QuantConnect](https://www.quantconnect.com/docs/v2/writing-algorithms/reality-modeling/transaction-fees/key-concepts?utm_s …
- L73: * **Settlement Model (ISettlementModel)**:
- L75: * 현금 결제 주기/규칙, 시간 경과에 따른 자금 적용(Scan/ApplyFunds 호출). ([[QuantConnect](https://www.quantconnect.com/docs/v2/writing-algorithms/reality-modeling/settlement/key-concepts?utm_source=cha …
...
- L81: * 다통화 포트폴리오 현금 장부·환율변환·총자산 반영. ([[QuantConnect](https://www.quantconnect.com/docs/v2/writing-algorithms/portfolio/cashbook?utm_source=chatgpt.com)][16], [[QuantConnect](https://www …
- L87: * 대차 가능한 주식 수량·대차비용 모델링 → 없으면 공매도 주문 거부. ([[QuantConnect](https://www.lean.io/docs/v2/lean-engine/class-reference/interfaceQuantConnect_1_1Interfaces_1_1IShortableProvider.html?utm …
...
- L61: - **EdgeOverride (DB, WVG scope):** 월드-로컬 도달성 제어 레코드. `(world_id, src_node_id, dst_node_id, active=false, reason)` 형태로 특정 월드에서 비활성화할 에지를 명시한다. 구현은 [`EdgeOverrideRepository`]({{ cod …
...
## /Users/munseungjin/workspace/research/qmtl/docs/ko/architecture/dag-manager.md
- Kind: ko_low_hangul — Hangul: 18.6%, Latin: 54.5%

Sample lines:
- L2: title: "QMTL DAG Manager — 상세 설계서 (Extended Edition)"
- L11: # QMTL DAG Manager — 상세 설계서 (Extended Edition)
- L16: - [Architecture Overview](README.md)
- L17: - [QMTL Architecture](architecture.md)
- L19: - [Lean Brokerage Model](lean_brokerage_model.md)
- L22: - 레퍼런스: [Commit‑Log 설계](../reference/commit_log.md), [TagQuery 사양](../reference/tagquery.md)
- L35: | **SRE Friendly**                    | gRPC/HTTP 인터페이스, 메트릭·로그·Alert 통합, Admin CLI          | §6, §10      |
- L42: ## 0-A. Ownership & Commit-Log Design
- L47: > Terminology / SSOT boundary: Global Strategy Graph(GSG, 전역 DAG)의 SSOT는 DAG Manager이며 불변(append‑only)이다. 월드‑로컬 객체(World View Graph=WVG: WorldNodeRef, Validation, DecisionsRequest) …
- L53: `"{partition_key(node_id, interval, bucket_ts)}:{input_window_hash}"`
- L68: | `ComputeNode`     | `node_id`(pk), `node_type`, `interval`, `period`, `code_hash`, `schema_compat_id`, `params_canon` | `created_at`, `tags[]`, `owner`, `schema_hash`         | D …
- L69: | `Queue`           | `topic`, `created_at`, `ttl`, `retention_ms`                    | `brokers`, `tag`, `lag_alert_threshold` | Kafka/Redpanda 토픽       |
- L70: | `VersionSentinel` | `version`, `commit_hash`, `created_at`                          | `release_tag`, `traffic_weight`         | 버전 경계 · 롤백 포인트          |
- L71: | `Artifact`        | `path`, `checksum`, `size`                                      | `framework`, `dtype`                    | 모델·파라미터 파일 등 binary     |
- L76: (ComputeNode)-[:EMITS]->(Queue)
- L77: (VersionSentinel)-[:HAS]->(ComputeNode)
- L78: (Artifact)-[:USED_BY]->(ComputeNode)
- L87: CREATE CONSTRAINT compute_pk IF NOT EXISTS ON (c:ComputeNode) ASSERT c.node_id IS UNIQUE;
- L88: CREATE INDEX kafka_topic IF NOT EXISTS FOR (q:Queue) ON (q.topic);
- L91: CREATE INDEX compute_tags IF NOT EXISTS FOR (c:ComputeNode) ON (c.tags);
