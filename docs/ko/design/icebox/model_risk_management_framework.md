...
- WorldService는 필요 시 `evaluation_run_created`/`evaluation_run_updated`/`validation_profile_changed` 등의 이벤트를 발행하고,  
...
- **“지표가 준비되면 호출되는 콜백 헬퍼”** 같은 편의 기능은,
  ...
  - 스트리밍 이벤트(`evaluation_run_updated` 등)를 구독하는 형태로 설계·도입 여부를 논의한다.
