...
!!! warning "Icebox (참고용, 현재 작업 대상 아님)"
    이 문서는 `docs/ko/design/icebox/`에 보관된 참고용 구현 계획서입니다. v1 구현 당시의 체크리스트/맥락을 남기기 위한 문서이며, 현재 SSOT는 `docs/ko/architecture/` 및 코드/테스트입니다.
...
### 6.2 인터페이스/배포 형태
- **이벤트**: ControlBus(CloudEvents) 이벤트 예시: `risk_snapshot_updated`, `evaluation_run_created`, `evaluation_run_updated`, `validation_profile_changed`. (그 외 지표별 세분 이벤트는 필요 시 확장)
...
- Exit engine: 동일 스냅샷 + market/risk 지표를 사용해 전략 exit 신호를 생성하고, 예: `exit_signal_emitted`로 발행.
...
- 스트레스/라이브 재시뮬레이션 워커: hub 이벤트(예: `risk_snapshot_updated`)를 트리거로 삼아 ControlBus/백그라운드 워커에서 실행하도록 스케줄링 훅을 연결한다.
...
- 단계별 작업 흐름 고정:
  1) **Hub 최소 구현**: 스냅샷 저장/조회 API + `risk_snapshot_updated` 이벤트 발행.
