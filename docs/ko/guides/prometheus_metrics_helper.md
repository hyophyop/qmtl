...
테스트에서는 프라이빗 속성(`_vals`, `_val`)에 직접 접근하지 말고 `metrics_factory` 헬퍼를 사용하세요.
...
공통 헬퍼를 사용하면 메트릭 구현이 바뀌어도 테스트가 안전하게 유지되고, 여러 서브시스템(Gateway/SDK/DAG Manager)에서 동일한 접근 패턴을 공유할 수 있습니다.
