...
관련: [ControlBus 운영 프로필 및 장애 대응](../operations/controlbus_operations.md)  
관련: [ControlBus/큐 운영 표준](../operations/controlbus_queue_standards.md)
...
!!! warning "런타임과 배포"
- dev/prod 운영 차이, 브로커/토픽 요구사항, lag/ACK/gap 장애 대응은 [ControlBus 운영 프로필 및 장애 대응](../operations/controlbus_operations.md)에서 다룬다.
- 토픽, consumer group, retry/DLQ, rehearsal 기준은 [ControlBus/큐 운영 표준](../operations/controlbus_queue_standards.md)을 따른다.
...
- 현재 구현의 apply 완료는 ACK 스트림 수렴을 하드 게이트로 차단하지 않는다. 기본 운영 가이드는 [ACK/Gap Resync RFC (초안)](../design/ack_resync_rfc.md)에서 관리한다.
...
운영 절차
- consumer group 재생성, 파티션 증설, reconcile/backfill 체크리스트는 [ControlBus 운영 프로필 및 장애 대응](../operations/controlbus_operations.md)과 [ControlBus/큐 운영 표준](../operations/controlbus_queue_standards.md)에서 유지한다.
