...
!!! abstract "요약"
    이 문서는 Core Loop의 **backend automation contract**를 설명한다.  
    사용자 관점의 golden path는 [Core Loop 계약](../contracts/core_loop.md)과 [월드 라이프사이클 계약](../contracts/world_lifecycle.md)에서 본다.  
    여기서는 단계/승격/거버넌스를 WorldService가 어떻게 소유하고, 평가 입력을 `risk_signal_hub` SSOT에 어떻게 고정하는지를 다룬다.

관련 문서:
- 제품 표면: [contracts/core_loop.md](../contracts/core_loop.md)
- 월드 단계 계약: [contracts/world_lifecycle.md](../contracts/world_lifecycle.md)
...
- 운영 승인/롤백: [operations/activation.md](../operations/activation.md)
- 운영 거버넌스: [operations/world_validation_governance.md](../operations/world_validation_governance.md)
...
- 전략/노드 자동 증류(icebox, 참고용): [design/strategy_distillation.md](../design/icebox/strategy_distillation.md)
...
이 문서는 사용자용 설명보다 **서비스 경계와 backend contract**를 우선한다.
...
- **Core Loop Hub 계층**: `qmtl/services/worldservice/core_loop_hub.py`가 Risk Hub 스냅샷을 읽어 평가용 메트릭을 파생하고, `risk_snapshot_updated` 라우팅/extended-validation 트리거를 소유한다.
...
제품 표면에서의 제출/결과 계약은 [Core Loop 계약](../contracts/core_loop.md)으로 분리한다.
...
- **metrics는 클라이언트에서 조립하지 않아도 된다.**
  ...
  - 우선순위(요약):
    ...
    - Core Loop Hub가 (paper/live/shadow/dryrun) `realized_returns`에서 v1 core 성과 지표를 계산해 채움
...
운영 절차와 승인 flow는 [월드 활성화 런북](../operations/activation.md)과 [World Validation 거버넌스](../operations/world_validation_governance.md)에서 다룬다.
