...
- 목적: 프리트레이드 점검, 주문 사이징, 실행, 체결 수집, 포트폴리오 업데이트, 리스크 제어, 타이밍 게이트 등 실행 레이어에서 사용하는 대표 노드들을 정리합니다.
- Core Loop 상 위치: Core Loop의 “전략 실행 및 주문 경로” 단계에서, 신호 이후 주문이 어떤 노드 체인을 통해 실행/피드백 되는지 이해하기 위한 레퍼런스입니다. 실전 전략에서는 보통 [거래소 노드 세트](exchange_node_sets.md)를 통해 간접적으로 사용합니다.
