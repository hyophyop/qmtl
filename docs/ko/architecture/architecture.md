...
이 문서는 QMTL의 **서비스 경계, 불변식, 도메인 계약**만 다룬다.

- 부트스트랩, 배포, config validate, 런북은 [운영 문서](../operations/README.md)에서 다룬다.
- 레거시 제거와 submit-only 표면으로의 전환은 [마이그레이션: 레거시 모드 및 하위 호환성 제거](../guides/migration_bc_removal.md)에서 다룬다.
- 현재 구현 범위와 `partial/implemented` 상태는 [QMTL 구현 추적성](implementation_traceability.md)에서 관리한다.

<a id="core-loop-summary"></a>
## Core Loop 합의 요약 (편입)

Core Loop의 제품 표면 계약은 별도 문서로 분리한다.

- 전략 작성자 관점의 golden path는 [Core Loop 계약](../contracts/core_loop.md)에서 정의한다.
- 단계 전이와 승격 handoff는 [월드 라이프사이클 계약](../contracts/world_lifecycle.md)에서 정의한다.
- 이 문서는 그 계약을 떠받치는 **서비스 경계와 내부 아키텍처**를 설명한다.
...
본 문서는 이러한 구조적 재사용성을 달성하기 위한 QMTL 아키텍처의 설계 철학, 계층 구조 정의, 컴포넌트 간 통신 계약, 상태 복원 메커니즘, 큐 오케스트레이션에 필요한 결정론적 조건을 규범 관점에서 정리한다.

QMTL 아키텍처 전반의 **핵심 설계 가치**는 다음 한 문장으로 요약된다.

> **“전략 로직에 집중하고, 시스템은 오케스트레이션·정책·안전한 자동화를 맡는다.”**

따라서 이 문서에서 정의하는 모든 컴포넌트(Gateway, DAG Manager, WorldService, SDK/Runner 등)는 다음 원칙을 공유한다.
- 전략 작성자는 **전략 DAG와 월드 선택** 정도에만 신경 쓰고, 나머지(단계 결정·큐 생성·스케일링·ExecutionDomain·Feature Artifact 관리·리스크/타이밍 게이트)는 시스템 내부 레이어가 맡는다.
- 내부 레이어는 복잡해질 수 있지만, 외부 인터페이스는 가능한 한 단일 제출과 읽기 전용 결과 surface를 중심으로 유지한다.
- 새로운 기능을 추가할 때도 사용자가 더 많은 옵션을 알게 만드는 대신, 시스템이 자동 결정하고 필요 시에만 override 하게 하는 방향을 기본으로 삼는다.

### 0-A. 설계 판단 기준 (규범)

이 문서는 현재 아키텍처를 설명하지만,
향후 기능 추가·변경의 판단 기준은 다음 문서들이 우선한다.

- [QMTL 설계 원칙](design_principles.md): 기능 추가 시 따라야 할 1급 규칙
- [QMTL Capability Map](capability_map.md): archetype이 아닌 capability 중심 구조
- [QMTL Semantic Types](semantic_types.md): legality와 격리 규칙의 판단 축
- [QMTL Decision Algebra](decision_algebra.md): 공통 decision family와 planner 경계

특히 다음 규칙은 아키텍처 개편과 기능 확장의 **규범적 기준**으로 간주한다.

- 새 기능은 archetype별 특례가 아니라 capability 조합으로 설명되어야 한다.
- 금지 규칙은 feature 이름이 아니라 semantic type으로 표현되어야 한다.
- `ML + MM` 같은 조합은 예외가 아니라 기본 합성 경로여야 한다.
- profile은 문서화와 온보딩을 위한 예시일 뿐, 코어 설계의 1급 축이 아니다.

따라서 향후 문서나 구현에서 조합별 특례가 늘어나기 시작하면,
구현 세부보다 먼저 capability 경계와 semantic contract를 재검토한다.

!!! note "정식 실행 모드 범위"
    이 문서에서 말하는 Core Loop와 실행 모델은 **항상 WorldService와 Gateway가 포함된 분산 스택**을 전제로 한다.  
    **WorldService/Gateway 없이 Runner/SDK만으로 전체 전략 생애 주기(제출→평가→게이팅→배포)를 처리하는 “순수 로컬 모드”는 정식 지원 모드가 아니다.**
...
Core Loop의 제품 서술은 [Core Loop 계약](../contracts/core_loop.md)과 [월드 라이프사이클 계약](../contracts/world_lifecycle.md)으로 분리했다.

이 문서에서 유지하는 요약은 다음뿐이다.

- 전략 제출은 `Runner.submit(..., world=...)` 하나로 수렴한다.
- 단계와 world 정책 판단은 Gateway/WorldService 경계를 통해 관리된다.
- 데이터 온램프와 replay/backtest는 데이터 플레인과 SDK orchestration이 담당한다.
- allocation/rebalancing은 별도 제어면 계약으로 분리되며, 자세한 내용은 [월드 할당 및 리밸런싱 계약](rebalancing_contract.md)을 따른다.

### 레거시와 마이그레이션

이 문서는 현재의 정식 표면만을 전제로 한다. 레거시 API 제거 원칙과 실제 체크리스트는 [마이그레이션: 레거시 모드 및 하위 호환성 제거](../guides/migration_bc_removal.md)에서 관리한다.
...
- Runner/SDK는 `shadow`를 별도 execution domain으로 유지하되, `shadow`에서 주문 발행 경로는 하드 차단된다. `offline`/`sandbox` 등의 모호 토큰은 backtest로 강등된다.
...
- 구현 메모: 현재 저장소/어댑터 범위는 [QMTL 구현 추적성](implementation_traceability.md)에서 관리한다. 이 문서의 규범은 “백테스트/드라이런은 기록 가능, 라이브/섀도우는 읽기 전용 소비”라는 계약 자체이다.
...
운영 신뢰성 제안, determinism 체크리스트, 관측·런북 연결은 [아키텍처 런타임 신뢰성](../reference/architecture_runtime_reliability.md)으로 분리한다. 본 문서는 런타임 튜닝 항목보다 서비스 경계와 불변식에 집중한다.
...
### 4.2 구현 예시와 데이터 on-ramp

전략 예시, Seamless on-ramp, TagQuery 예시, 교차 시장 예시는 [QMTL 아키텍처 예시](architecture_examples.md)로 분리한다. 월드 기반 데이터 wiring 계약은 [월드 데이터 preset 계약](../world/world_data_preset.md)에서 관리한다.
...
- [아키텍처 런타임 신뢰성](../reference/architecture_runtime_reliability.md): determinism 체크리스트, 운영 신뢰성 제안, 관측 연결
- [QMTL 아키텍처 예시](architecture_examples.md): 전략 예시와 data on-ramp 예시
- [월드 이벤트 스트림 런타임](world_eventstream_runtime.md): activation/queue/policy fan-out 경계
- [ACK/Gap Resync RFC (초안)](../design/ack_resync_rfc.md): ACK/gap resync 설계 논점
