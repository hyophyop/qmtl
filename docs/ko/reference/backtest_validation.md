...
러너 통합:
- 로컬 백테스트의 경우 `Runner.submit(MyStrategy)`를 사용하고, 설정 또는 사전 점검 단계에서 검증을 호출합니다.
- WS 우선 실행의 경우 `Runner.submit(MyStrategy, world="name")`를 사용하고, 필요하면 검증을 별도의 프리플라이트 단계로 유지합니다.
