...
- activation 엔벌로프에는 `as_of`가 없으므로 safe-mode 평가가 `paper`를 `backtest`로 강등합니다(`downgraded=true`, `downgrade_reason=missing_as_of`, `safe_mode=true`). 반대로 `shadow`는 missing-`as_of` 가드로 강등되지 않습니다.
...
- SDK는 `effective_mode`/`execution_domain`을 로컬 상태·메트릭용 읽기 전용 주석으로 취급해야 하며, 이 값으로 백엔드 결정을 덮어쓰거나 클라이언트 실행 동작을 바꾸면 안 됩니다.
