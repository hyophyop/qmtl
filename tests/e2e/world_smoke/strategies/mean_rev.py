...
executes a no-op Strategy via Runner.submit in backtest mode to
...
def run_qmtl_mode(world_id: str, duration_sec: int = 5):
    ...
    WS-first API에 맞춰 Runner.submit(backtest)만 사용합니다.
    QMTL이 import 불가하면 SDK fallback으로 대체합니다.
    """
    try:
        from qmtl.runtime.sdk import Mode, Runner, Strategy  # type: ignore
    except Exception:
    ...
    class _NoopStrategy(Strategy):  # minimal offline-compatible strategy
        def setup(self) -> None:  # noqa: D401
            ...
            # Runner.submit(backtest)는 실행 가능한 노드가 없으면 곧바로 종료합니다.
            return None
    ...
    # Backtest 모드로 간단 제출 (실제 노드 실행 없음)
    try:
        Runner.submit(
            _NoopStrategy,
            world=world_id,
