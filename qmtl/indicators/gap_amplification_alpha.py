...
__all__ = ["gap_amplification_node"]


def _level_series(view: CacheView, time: int, side: str, feature: str) -> list[Any]:
    """Return ``feature`` values across levels for ``side`` at ``time``."""
    try:
        levels = view[time][side]
        data = getattr(levels, "_data", levels)
    ...
    for lvl in sorted(data):
        try:
            feat = levels[lvl][feature]
            values.append(getattr(feat, "_data", feat))
        except Exception:  # pragma: no cover - defensive
            continue
    return values
...
def gap_amplification_node(data: dict, view: CacheView | None = None) -> dict:
    ...
    if view is not None:
        try:
            if ask_gaps is None:
                ask_gaps = _level_series(view, time_idx, "ask", "gap")
            if ask_depths is None:
                ask_depths = _level_series(view, time_idx, "ask", "depth")
            if bid_gaps is None:
                bid_gaps = _level_series(view, time_idx, "bid", "gap")
            if bid_depths is None:
                bid_depths = _level_series(view, time_idx, "bid", "depth")
            if hazard is None:
                ofi = _value_at(view, time_idx, "hazard", 0, "ofi", ofi)
                spread_z = _value_at(view, time_idx, "hazard", 0, "spread_z", spread_z)
                if eta is None:
                    eta0 = _value_at(view, time_idx, "hazard", 0, "eta0")
                    eta1 = _value_at(view, time_idx, "hazard", 0, "eta1")
                    eta2 = _value_at(view, time_idx, "hazard", 0, "eta2")
