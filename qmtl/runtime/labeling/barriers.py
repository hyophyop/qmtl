...
from qmtl.runtime.labeling.schema import BarrierSpec
...
def volatility_scaled_barrier_spec(
    *,
    ...
    """Return a volatility-scaled :class:`BarrierSpec` in absolute price units.

    The barrier levels are fixed at ``price`` and expressed as absolute price
    thresholds. ``sigma`` is expected to be a return volatility (e.g., stdev of
    returns), making ``sigma * price`` the base price distance.
    """
