...
_SCHEMAS_DIR = Path(__file__).resolve().parents[2] / "docs" / "reference" / "schemas"

ORDER_PAYLOAD_SCHEMA = (_SCHEMAS_DIR / "order_payload.schema.json").read_text()
ORDER_ACK_SCHEMA = (_SCHEMAS_DIR / "order_ack.schema.json").read_text()
EXECUTION_FILL_EVENT_SCHEMA = (_SCHEMAS_DIR / "execution_fill_event.schema.json").read_text()
PORTFOLIO_SNAPSHOT_SCHEMA = (_SCHEMAS_DIR / "portfolio_snapshot.schema.json").read_text()
