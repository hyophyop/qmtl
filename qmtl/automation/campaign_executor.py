...
class CampaignExecutor:
    ...
    Evaluate note
    -------------
    The executor can optionally run evaluate calls, but it does not attempt to
    build metrics client-side. WorldService is responsible for sourcing metrics
    when missing (e.g., from stored evaluation runs or risk hub snapshots).
    """
        ...
        for raw in raw_actions[: max(0, int(cfg.max_actions or 0))]:
            ...
            if method in {"POST", "PUT"}:
                if isinstance(suggested_body, Mapping):
                    is_evaluate = path.endswith("/evaluate") or path.endswith("/evaluate-cohort")
                    if is_evaluate and not cfg.execute_evaluate:
                        results.append(
                            ExecutionResult(
                                action=action,
                                method=method,
                    ...
                    if path.endswith("/evaluate"):
                        sid = cfg.strategy_id or str(raw.get("strategy_id") or suggested_body.get("strategy_id") or "")
                        if not sid:
                            results.append(
                                ExecutionResult(
                                    action=action,
