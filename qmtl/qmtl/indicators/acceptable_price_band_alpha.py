...
__all__ = ["acceptable_price_band_node"]

# Shared cache for history across invocations.
CACHE = FourDimCache()
...
def acceptable_price_band_node(
    ...
    volume_hat = data.get("volume_hat", _cached_avg("volume_hat", 0.0))
    volume_std = data.get("volume_std", _cached_avg("volume_std", 1.0))
    ...
    for feature, value in [
        ...
        if not isinstance(values, deque) or values.maxlen != cache_window:
            values = (
                deque(values, maxlen=cache_window)
                if isinstance(values, deque)
                else deque(maxlen=cache_window)
            )
        values.append(value)
