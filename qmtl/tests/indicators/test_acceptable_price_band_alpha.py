...
from qmtl.indicators.acceptable_price_band_alpha import acceptable_price_band_node


def test_band_calculation_and_alpha():
    cache = FourDimCache()
    data = {
        "price": 110.0,
        "volume": 150.0,
    ...
    assert result["band_upper"] == pytest.approx(109.94, rel=1e-3)
    assert result["pbx"] == pytest.approx(1.012, rel=1e-3)
    assert result["alpha"] > 0


def test_alpha_signs_under_varied_inputs():
...
def test_cache_retrieval_and_window():
    ...
    for feature in ("mu", "sigma", "volume_hat", "volume_std"):
        ...
        manual_cache.set(
            "t0", "mid", 100, feature, deque(values, maxlen=values.maxlen)
        )
    ...
    assert len(third["cache"].get("t0", "mid", 100, "mu")) == 2
