...
이 저장소는 전략 개발과 동시에 QMTL 자체의 문제/개선을 즉시 반영하기 위한 목적을 갖습니다. 따라서 `qmtl` 서브모듈은 항상 원격 최신 상태를 기준으로 작업해야 합니다.

- 작업(탐색, 코딩, 테스트) 시작 전 반드시 서브모듈을 최신으로 동기화하세요.
  ```bash
  git submodule update --init --recursive --remote
  # 브랜치 추적이 불확실할 때(혹은 고정되어 있을 때) 강제 최신화
  git -C qmtl fetch --all --prune
  git -C qmtl checkout main || true
  git -C qmtl pull --ff-only || true
  ```
- 서브모듈 갱신으로 포인터가 변경되면 루트 리포지토리에 포인터 변경을 커밋하세요.
  ```bash
  git add qmtl
  git commit -m "chore: bump qmtl submodule to latest"
  ```
- 전략 개발 중 QMTL의 버그/개선이 확인되면, 우선 `qmtl/` 서브모듈 내에 수정과 테스트를 추가하고, 루트 전략 테스트도 함께 갱신합니다. 변경 사항을 반영한 뒤 위 절차로 포인터를 업데이트하세요.
- PR 체크리스트에 다음을 포함하세요.
  - [ ] `git submodule status --recursive` 결과가 원격 최신과 일치함을 확인
...
# 원격 최신과 로컬 포인터 비교(요약)
git -C qmtl log --oneline -n 3 && git -C qmtl rev-parse --abbrev-ref HEAD
```
...
- 전략 개발 중 QMTL 이슈가 발견되면, 서브모듈을 최신으로 동기화한 뒤 `qmtl/` 내부에서 우선 수정하고 테스트하세요. qmtl은 서브모듈이지만 별도 구성된 프로젝트이므로 `qmtl/AGENTS.md`의 가이드를 충실히 따라 수정/테스트/커밋/PR을 진행해야 합니다. 수정 후 루트 리포지토리에 서브모듈 포인터 업데이트를 커밋하는 것을 잊지 마세요.
